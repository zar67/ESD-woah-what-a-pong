...
public:
    ...
    // The angle the ball should go off at is calculated depending on where the ball hits the paddle
    // Returns the new direction.
    Vector calculateNewDir(Player paddle, float y, float y_dir, float size, float speed);

    // Used to see if the two lines intersect.
    // Returns the point of intersection.
    Vector intersect(Vector p1, Vector p2, Vector q1, Vector q2);

    // Uses ray casting and bounding boxes to detect if given positions have hit anything.
    // The parameters are passed in instead of the ball object because both the AI ball and the game ball need to be checked for collisions.
...
private:
    ...
    // The AI ball moves at a faster speed and the right paddle positions itself on the same Y, giving the illusion it is trying to predict the ball's trajectory.
    Ball ai_ball;

    // Player one is always player controlled, player two can be either player or 'computer' controlled.
